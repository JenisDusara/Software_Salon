...
function KpiCard({
  icon, iconBg, value, label, sub, trend, valueColor = "text-[#1C1917]",
}: {
...
function StatusBadge({ status }: { status: AppointmentStatus }) {
  ...
  const labels: Record<AppointmentStatus, string> = {
    CONFIRMED: "Confirmed", COMPLETED: "Completed", IN_PROGRESS: "In Progress", PENDING: "Pending",
  };
  return (
    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${map[status]}`}>{labels[status]}</span>
  );
}
    ...
    <div className="bg-white border border-[#E7E5E4] rounded-xl shadow-lg p-3 text-sm min-w-[180px]">
      ...
      <div className="space-y-1">
        <div className="flex justify-between gap-4">
          <span className="flex items-center gap-1.5 text-[#78716C]"><span className="w-2 h-2 rounded-full bg-[#059669] inline-block" />Revenue</span>
          <span className="font-medium text-[#1C1917]">{formatINR(revenue)}</span>
        </div>
        <div className="flex justify-between gap-4">
          <span className="flex items-center gap-1.5 text-[#78716C]"><span className="w-2 h-2 rounded-full bg-[#DC2626] inline-block" />Expenses</span>
        ...
        <div className="flex justify-between gap-4 border-t border-[#E7E5E4] pt-1 mt-1">
          ...
          <span className={`font-semibold ${revenue - expenses >= 0 ? "text-[#059669]" : "text-[#DC2626]"}`}>{formatINR(revenue - expenses)}</span>
        </div>
      </div>
    </div>
  );
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([s, a, inv, monthly, payment, services]) => {
      setStats(s);
  ...
  return (
    <div className="space-y-6 p-6 bg-[#FAFAF9] min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold font-heading text-[#1C1917]">Good morning 👋</h1>
          <p className="text-sm text-[#78716C] mt-1">Here&apos;s what&apos;s happening at your salon today</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right hidden sm:block">
            <p className="text-sm font-medium text-[#1C1917]">{today.toLocaleDateString("en-IN", { weekday: "long" })}</p>
      ...
      <div className="bg-white rounded-xl border border-[#E7E5E4] shadow-sm p-5">
        ...
        {monthlyData.length === 0 ? (
          <div className="h-[320px] flex items-center justify-center text-[#78716C] text-sm">No revenue data yet</div>
        ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 bg-white rounded-xl border border-[#E7E5E4] shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-semibold font-heading text-[#1C1917]">Today&apos;s Appointments</h2>
            <button className="text-xs text-[#D97706] hover:underline font-medium">View All →</button>
          </div>
          <div className="space-y-3">
            ...
            {todayAppts.map((appt) => (
              <div key={appt.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-[#FAFAF9] transition-colors border border-transparent hover:border-[#E7E5E4]">
                ...
                <div className="hidden sm:flex items-center gap-1.5 shrink-0">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-white text-[9px] font-bold ${getAvatarColor(appt.staffName)}`}>
                    {getInitials(appt.staffName)}
                  </div>
                  <span className="text-xs text-[#78716C] hidden md:block">{appt.staffName.split(" ")[0]}</span>
                </div>
                <div className="shrink-0"><StatusBadge status={appt.status as AppointmentStatus} /></div>
                <div className="shrink-0 text-right flex items-center gap-2">
                  <span className="text-sm font-semibold text-[#1C1917]">{formatINR(appt.amount)}</span>
                  {appt.status === "COMPLETED" && (
                    <button className="text-xs text-[#D97706] border border-[#D97706] hover:bg-amber-50 px-2 py-0.5 rounded-lg font-medium transition-colors">Bill</button>
                  )}
                </div>
              </div>
            ))}
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          ...
          <div className="bg-white rounded-xl border border-[#E7E5E4] shadow-sm p-5 flex-1">
            <h3 className="text-sm font-semibold font-heading text-[#1C1917] mb-4">Top Services This Month</h3>
                  ...
                  return (
                    <div key={svc.name}>
                      ...
                      <div className="h-1.5 bg-[#F5F5F4] rounded-full overflow-hidden">
                        <div className="h-full bg-[#D97706] rounded-full transition-all" style={{ width: `${pct}%` }} />
                      </div>
                    </div>
                  );
                })}
      ...
      <div className="bg-white rounded-xl border border-[#E7E5E4] shadow-sm p-5">
            ...
            <thead>
              <tr className="border-b border-[#E7E5E4]">
                {["Invoice #", "Client", "Services", "Amount", "Status", "Action"].map((col) => (
                  <th key={col} className="text-left text-xs font-medium text-[#78716C] pb-3 pr-4 last:pr-0">{col}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              {recentInvoices.map((inv) => (
                <tr key={inv.id} className="border-b border-[#F5F5F4] last:border-0 hover:bg-[#FAFAF9] transition-colors">
                  ...
                  <td className="py-3 pr-4 max-w-[200px]"><span className="text-xs text-[#78716C] line-clamp-1">{inv.items.map((it) => it.description).join(", ")}</span></td>
                  ...
                  <td className="py-3">
                    <button className="text-xs text-[#78716C] border border-[#E7E5E4] hover:border-[#D97706] hover:text-[#D97706] px-3 py-1.5 rounded-lg transition-colors font-medium">View</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="md:hidden space-y-3">
          ...
          {recentInvoices.map((inv) => (
            <div key={inv.id} className="flex items-center gap-3 p-3 rounded-xl border border-[#E7E5E4]">
